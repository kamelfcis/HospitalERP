...
export const lineTypeSpecs: Record<string, Record<string, LineTypeSpec>> = {
  ...
  sales_invoice: {
    ...
    // ── فواتير التعاقد (صيدلية) ─────────────────────────────────────────────
    pharmacy_patient_receivable:  { required: "cond", condition: "ذمة نصيب المريض في فواتير التعاقد — حساب أصل (مدين عند البيع)", debitSide: true, creditSide: true },
  ...
  patient_invoice: {
    ...
    revenue_services:    { required: "cond", condition: "عند وجود خدمات طبية عامة",                             debitSide: false, creditSide: true },
