...
// ─────────────────────────────────────────────────────────────────────────────
// InvoiceLineRow — مُحسَّن بـ React.memo
// يُعيد render فقط عند تغيير البيانات الخاصة بهذا السطر
// ─────────────────────────────────────────────────────────────────────────────
...
export function InvoiceLineTable({
  ...
  // ── كشف الأصناف ذات الأسعار المتعددة (memoized — يُحسَب فقط عند تغيير lines) ─
  const multiPriceItems = useMemo(() => {
    ...
    lines.forEach((ln) => {
      ...
      if (same.length > 1) {
        ...
        if (prices.size > 1) result.add(ln.itemId);
      }
      // كشف 2: خيارات الصلاحية تحتوي على أسعار مختلفة
      ...
      if (opts && opts.length >= 1) {
        ...
        for (const pr of byExpiry.values()) {
          if (pr.size > 1) { result.add(ln.itemId); break; }
        }
        // كشف 3: hasPriceConflict من الباكند
        if (opts.some((o) => o.hasPriceConflict)) result.add(ln.itemId);
        // كشف 4: أسعار مختلفة عبر تواريخ صلاحية مختلفة
        const allPrices = new Set(opts.map((o) => o.lotSalePrice || "0").filter((p) => p !== "0"));
        if (allPrices.size > 1) result.add(ln.itemId);
      }
    });
    return result;
  }, [lines]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // ابحث عن العنصر المستهدف — وإن لم يوجد العمود، ابحث عن أقرب عمود
