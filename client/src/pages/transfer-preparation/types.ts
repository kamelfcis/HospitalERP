...
export type SortDir = boolean | null;
...
export function toMajor(qtyMinor: number, majorToMinor: number): number {
  ...
  return Math.round((qtyMinor / majorToMinor) * 10000) / 10000;
}
...
export function valInMajor(line: PrepLine, field: BulkField): number {
  return toMajor(parseFloat(line[field]) || 0, getMajorToMinor(line));
}
