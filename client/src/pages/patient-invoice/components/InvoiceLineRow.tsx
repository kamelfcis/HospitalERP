...
const LINE_TYPE_CONFIG: Record<KnownLineType, { label: string; color: string }> = {
  service:     { label: "خدمة", color: "text-blue-700 dark:text-blue-400" },
  drug:        { label: "دواء", color: "text-green-700 dark:text-green-400" },
  consumable:  { label: "مستهلك", color: "text-orange-700 dark:text-orange-400" },
  equipment:   { label: "جهاز", color: "text-purple-700 dark:text-purple-400" },
  doctor_cost: { label: "طبيب", color: "text-rose-700 dark:text-rose-400" },
};

function getLineTypeLabel(lineType: string, isReadOnly: boolean, isCostLine: boolean) {
  const cfg = LINE_TYPE_CONFIG[lineType as KnownLineType];
  const label = cfg?.label ?? lineType;
  const color = cfg?.color ?? "text-muted-foreground";
  const suffix = isReadOnly ? (isCostLine ? " ⓒ" : " ⓐ") : "";
  return <span className={`text-[10px] font-semibold whitespace-nowrap ${color}`}>{label}{suffix}</span>;
}
