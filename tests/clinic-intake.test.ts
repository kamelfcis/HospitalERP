...
const adminApi  = new AuthenticatedApi("admin", "admin123");
...
async function createTestAppointmentInClinicB(): Promise<string> {
  ...
  const result = await db.execute(sql`
    INSERT INTO clinic_appointments (clinic_id, doctor_id, patient_name, appointment_date, status)
    VALUES (${CLINIC_B}, ${DOCTOR_ID_B}, 'مريض اختبار عزل', CURRENT_DATE, 'waiting')
    RETURNING id
  `);
...
beforeAll(async () => {
  ...
  // Create temp users
  testReceptionUserId = await createUser(adminApi, "test_intake_reception", "reception", "استقبال اختبار");
  testDoctorUserId    = await createUser(adminApi, "test_intake_doctor",    "doctor",    "طبيب اختبار");
  testNonClinicUserId = await createUser(adminApi, "test_intake_noclinic",  "reception", "استقبال بلا عيادة");
  ...
  // Build API clients
  receptionApi = new AuthenticatedApi("test_intake_reception", TEMP_PASS);
  doctorApi    = new AuthenticatedApi("test_intake_doctor",    TEMP_PASS);
  noClinicApi  = new AuthenticatedApi("test_intake_noclinic",  TEMP_PASS);
