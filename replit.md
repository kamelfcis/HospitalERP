...
- Lot-level sale price tracking: Each inventory lot stores its receiving sale price (salePrice column on inventoryLots). Pricing hierarchy: department price > lot salePrice > item card salePriceCurrent. postReceiving writes line.salePrice into lot. FEFO preview returns lotSalePrice per allocation. Transfers carry lot salePrice to destination lots. Sales Invoice FEFO lines use per-lot pricing unless department price overrides. /api/pricing accepts optional lotId parameter and returns { price, source: "department"|"lot"|"item" }.
...
- Services & Price Lists module (/services-pricing): Two-tab page (Services Master + Price Lists). Services CRUD with department/category/serviceType filters, pagination, and toggle-active. Price Lists with split-view (left: list cards, right: items grid with inline price editing). Supports bulk price adjustment (PCT/FIXED, INCREASE/DECREASE, optional department/category filters, createMissingFromBasePrice), copy-from-list, and add-prices-from-services. Bulk adjust uses ROUND(expr, 2) in SQL, rejects negative prices. Tables: services, priceLists, priceListItems, priceAdjustmentsLog, serviceConsumables. 14 vitest tests for bulk adjustment calculations.
- Service Consumables: Links consumable items to services with predefined quantities and unit levels. When creating/editing a service, users can add consumable items (e.g., CBC test → 3cc syringe, urine analysis → collection cup). In Sales Invoice, "خدمة + مستهلكات" button opens service search modal; selecting a service auto-adds its consumables as invoice lines with FEFO allocation and stock sufficiency warnings. Table: service_consumables (serviceId, itemId, quantity, unitLevel, notes) with unique index on (serviceId, itemId). API: GET/PUT /api/services/:id/consumables (bulk replace pattern). addItemToLines accepts optional overrides {qty, unitLevel} so consumable-defined quantities and unit levels are honored (not defaulting to qty=1/major). Backend validates unitLevel ∈ {major, medium, minor} and quantity > 0.
