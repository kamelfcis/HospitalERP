...
- **Cashier & Security**: Includes real-time SSE for instant invoice visibility, password-protected cash drawers with GL account selection for shifts, department-level invoice isolation (each unit sees only its own pending invoices), a two-stage journal entry system for sales. Cashier screen fully refactored into hooks + compound components (925→~258 lines). Supports pharmacies AND departments as cashier units. **Multi-shift support**: users can have simultaneous open shifts for different units, switch between them via `ShiftSwitchBar`, and open new shifts while one is already active. **Close-shift validation** (Rule A/B): close blocked if pending docs + no other open shift; allowed if another shift exists for same unit. `cashier.view_shift_totals` permission gates `ShiftTotalsWidget` visibility.
