...
- **Permission Groups Management**: Admin UI for managing groups, members, and per-module permissions. Separate permissions for Bed Board (`bed_board.view`), Room Management (`rooms.manage`), and Surgery Types (`surgery_types.manage`). Formal permission model: `Effective = role_permissions ∪ group_permissions` (additive, no deny). PermissionsMatrixTab shows role-inherited permissions as locked (blue lock icon). User Effective Permissions Dialog (`GET /api/users/:id/effective-permissions`) shows each permission with source attribution (role/group/both). System groups use `system_key` to resolve role inheritance; custom groups return `rolePermissions: []`.
