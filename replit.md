...
- **Performance Optimization Session (Phases 1–4) — 2026-03-29**:
  - **Phase 1 — Item Movement Report Pagination**: Full server-side pagination (50 rows/page). Backend (`item-movement-report-storage.ts`): 3-query approach — COUNT(*), summary aggregation (all pages), paginated rows with window function for running balance (window evaluates before LIMIT so balance always correct). Route (`reports.ts`): accepts `page`/`pageSize`, export uses `pageSize: 50_000`. Frontend (`item-movement-report/index.tsx`): `page` state, derived `queryUrl` via `useMemo`, `SummaryBar` uses server-side `summary` (covers all pages), pagination controls (ChevronRight/Left), row counter `X–Y من Z حركة`, row numbering accounts for page offset. `staleTime: 5*60_000` on `/api/warehouses` within that page.
  - **Phase 2 — DB Index**: `idx_lot_movements_ref ON inventory_lot_movements (reference_type, reference_id)` added to startup migrations block in `server/index.ts`.
  - **Phase 3 — staleTime for Pharmacy Master Data**: `staleTime: 20_000` on sales-invoices list (`useRegistry.ts`). `staleTime: 5*60_000` on: `/api/warehouses` in `sales-invoices/index.tsx`, `supplier-receiving/index.tsx`, `sales-returns/SearchView.tsx`, `stock-count/index.tsx`, `store-transfers/useTransferLog.ts`; `/api/suppliers` (full list) in `purchase-invoices/index.tsx` and `supplier-receiving/index.tsx`; `/api/sales-invoices/pharmacists` in `useRegistry.ts`.
  - **Phase 4 — InvoiceLineTable Render Optimization**: Extracted `InvoiceLineRow` as `React.memo` component (shallow compare) from `sales-invoices/components/InvoiceLineTable.tsx`. `multiPriceItems` Set moved to `useMemo` (was O(n²) per render, now only recomputes when `lines` changes). `openStats` and `closeStats` in `useStatsDialog.ts` wrapped with `useCallback([], [])` — all props to InvoiceLineRow are now stable references. Result: tgayyur fi satr waahid → re-render for that row only (was all n rows).
