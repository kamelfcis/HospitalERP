...
- **Contracts Module — Phase 3 Claims GL Accounting (COMPLETE)**: `contractClaimBatches` + `contractClaimLines` tables with enum statuses (`draft/submitted/responded/settled/cancelled` and `pending/approved/rejected/settled`). Claim generator `server/lib/contract-claim-generator.ts` — fires automatically (fire-and-forget) on patient invoice finalization, creates/finds draft batch per company+contract, upserts claim lines idempotently from `companyShareAmount > 0` invoice lines. Claims storage mixin `server/storage/contracts-claims-storage.ts` with full lifecycle methods. 5 new API routes: GET list/single, PATCH submit, POST respond (per-line accept/reject), POST settle (generates GL journal DR:cash CR:company-AR), PATCH cancel. 3 new RBAC permissions: `contracts.claims.view / manage / settle`. Admin UI: `/contract-claims` page with two-panel RTL layout (batch list + detail with lines table), action buttons per batch status, respond dialog (per-line approve/reject with amount/reason), settle dialog (date + bank/AR accounts + reference). Sales Invoice / Clinic / Stay integrations deferred to Phase 4.
