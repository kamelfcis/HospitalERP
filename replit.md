...
- **Services & Price Lists**: CRUD operations for services (department-scoped), price lists with inline editing and bulk adjustments, and integration with sales invoices. **Refactored (Phase 17)** into `client/src/pages/services-pricing/` — `ServicesTab`, `ServiceDialog`, `PriceListsTab`, `PriceListModal`, `AddPricesModal`, `CopyFromModal`, `BulkAdjustModal`, `hooks`.
