...
- **Bed Board + Reception**: `floors` → `rooms` → `beds` tables with status lifecycle: EMPTY / OCCUPIED / NEEDS_CLEANING / MAINTENANCE. All mutations use `SELECT ... FOR UPDATE` row locks to prevent races. `admitPatientToBed` atomically creates admission + draft invoice + stay segment + marks bed OCCUPIED. `transferPatientBed` locks both beds in deterministic order (deadlock-safe), closes old stay segment, opens new one, source becomes NEEDS_CLEANING. `dischargeFromBed` closes stay segment + discharges admission + sets bed NEEDS_CLEANING. `setBedStatus` handles EMPTY / MAINTENANCE transitions; blocks changes on OCCUPIED beds. Each mutation emits `[BED_BOARD]` marker after commit + writes audit log. Frontend at `/bed-board`: floor/room/bed grid, status-color cards (green/blue/amber/red), controlled DropdownMenu per bed (no DOM hacks), Reception Sheet for admit (patient search + manual entry + department + service + doctor), Transfer Dialog (empty-bed selector), Discharge confirmation dialog. Auto-refreshes every 30 seconds. APIs: `GET /api/bed-board`, `GET /api/beds/available`, `POST /api/beds/:id/admit`, `POST /api/beds/:id/transfer`, `POST /api/beds/:id/discharge`, `POST /api/beds/:id/status`.
