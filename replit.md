...
- **Permission Group Discount Limits**: `permission_groups` table extended with `maxDiscountPct` (decimal 5,2) and `maxDiscountValue` (decimal 12,2). The effective limit is the most restrictive of user-level and group-level settings. Enforced server-side at finalize time (`/api/sales-invoices/:id/finalize`) with Arabic error messages. UI: real-time warnings in `InvoiceTotals` for both pct and value overages, editable in GeneralTab of Permission Groups page (system groups can update discount limits).
- **Permission Group Default Route**: `permission_groups` table extended with `defaultRoute` (varchar). After login, users are redirected to their group's designated screen instead of the dashboard. Configurable via a dropdown in GeneralTab with all major screens listed. System groups can also have a default route set. Managed via `/api/auth/me` which resolves and returns the group's `defaultRoute`.
