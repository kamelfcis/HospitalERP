...
- **Patient Master Linkage (Upgrade)**: Safe, non-breaking upgrade to unify patient identity across all modules. Added `patient_id` (nullable FK) to `sales_invoice_headers` (pharmacy invoices) — previously the only table missing it. Schema: `patientId` field + `idx_sales_inv_patient_id` index. `patientCode` now has UNIQUE constraint. `admissions` table has `idx_adm_patient_id` index. Full frontend chain: `useInvoiceForm` → `useInvoiceMutations.buildHeader()` → backend route → `createSalesInvoice`/`updateSalesInvoice` storage. `useLoadInvoice` now reads `patientId` from invoice detail. Backfill endpoint `POST /api/admin/backfill-pharmacy-invoice-patients` links existing invoices to patients by exact name match (unambiguous only). What remains intentionally unchanged: cash pharmacy sales still allow no patient (optional linkage), findOrCreatePatient service, dedup scoring engine, merge API, PatientSearchCombobox.
- **Patient Audit Trail**: Added `patient_assigned_at` (TIMESTAMPTZ) and `patient_assigned_by` (VARCHAR) to `sales_invoice_headers`. Storage layer sets these fields when patientId is first assigned (create: always when patientId present; update: only when transitioning from null → non-null). Tracks who linked the patient and when, for internal audit and tamper detection.
- **Progressive Patient Validation**: Finalize route now enforces `PATIENT_REQUIRED` (HTTP 422) for all non-cash invoice types (credit/contract/delivery) that lack `patientId`. Cash invoices remain unrestricted.
- **Backend Auto-Resolve Patient**: POST and PATCH `/api/sales-invoices` routes auto-call `findOrCreatePatient(customerName)` when `customerType !== "cash"` and `patientId` is null but `customerName` is present. Non-fatal (does not block save on failure).
- **Patient Financial Summary API**: `GET /api/patients/:id/financial-summary` returns aggregated: `totalAmount`, `totalPaid`, `totalOutstanding`, `invoiceCount`, `admissionCount`, `lastInteraction`, and a `breakdown` by pharmacy vs medical invoices. Pharmacy: uses `net_total` with cash=paid / non-cash=outstanding logic. Medical: uses `total_amount` and `paid_amount` from `patient_invoice_headers`.
- **Business Classification لبنود فاتورة المريض**: Separate nullable `business_classification` VARCHAR field on `items`, `services`, and `patient_invoice_lines` tables — fully decoupled from `category`/`serviceType`/`lineType`. Central resolver at `shared/resolve-business-classification.ts` (`resolveBusinessClassification` + `resolveBusinessClassificationClient`). 10 values: drug/consumable/equipment/gas/medical_service/lab/radiology/accommodation/operating_room/device. Auto-derived when adding lines (client-side via resolver), Stay Engine always sets 'accommodation'. Immutable after finalize (entire invoice is locked). Backfill endpoint `POST /api/admin/backfill-business-classification`. Badge shown in UnifiedLinesTab below the lineType badge (gray "غير مصنف" when null). Master data UI: ServiceDialog and ItemFormFields have a businessClassification dropdown. CLASSIFICATION_FALLBACK_USED warning logged when falling back to serviceType/lineType.
