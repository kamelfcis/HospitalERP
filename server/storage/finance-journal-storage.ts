...
const methods = {
  ...
  // ==================== Patient Invoice Journal — قيد فاتورة المريض المخصص =======
  //
  // هيكل القيد الناتج (مدين / دائن):
  // ┌─────────────────────────────────────────────────────────────────┐
  // │  مدين   │ النقدية (cash.debitAccountId)       = صافي الفاتورة  │ ← مرضى النقدي
  // │  مدين   │ الذمم (receivables.debitAccountId)  = صافي الفاتورة  │ ← مرضى التعاقد
  // ├─────────────────────────────────────────────────────────────────┤
  // │  دائن   │ إيراد خدمات (revenue_services.creditAccountId)      │
  // │  دائن   │ إيراد أدوية  (revenue_drugs.creditAccountId)        │
  // │  دائن   │ إيراد مستلزمات (revenue_consumables.creditAccountId)│
  // │  دائن   │ إيراد معدات  (revenue_equipment.creditAccountId)    │
  // ├─────────────────────────────────────────────────────────────────┤
  // │  مدين   │ تكلفة طبيب  (doctor_cost.debitAccountId)            │ ← إن وجد
  // │  دائن   │ مستحقات طبيب (doctor_cost.creditAccountId)          │
  // └─────────────────────────────────────────────────────────────────┘
  //
  // التوازن: Dr (cash|receivables) = Cr (sum of revenues)
  //          Dr (doctor_cost) = Cr (doctor_cost)   ← زوج مستقل
  //
    ...
    return await db.transaction(async (tx) => {
      ...
      if (journalLineData.length === 0) {
        logAcctEvent({
          ...
          status:       "needs_retry",
          errorMessage: `جميع سطور المعاملة "${sourceType}" غير مربوطة بحسابات — لم يُنشأ أي قيد. عرِّف ربط الحسابات من /account-mappings.`,
        }).catch(() => {});
        return null;
      }
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.01) {
        logAcctEvent({
          ...
          status:       "failed",
          errorMessage: `رُفض القيد: ميزان غير متوازن لفاتورة المريض (مدين=${totalDebit.toFixed(2)}, دائن=${totalCredit.toFixed(2)}). الربط الناقص: [${unmappedTypes.join(', ')}]. تأكد من ربط جميع بنود الإيراد.`,
        }).catch(() => {});
        return null;
      }
      ...
      if (!periodId) {
        logAcctEvent({
          ...
          status:       "needs_retry",
          errorMessage: `لا توجد فترة مالية مفتوحة تغطي تاريخ ${params.entryDate} — رُفض إنشاء القيد. افتح فترة مالية تشمل هذا التاريخ ثم أعد الترحيل.`,
        }).catch(() => {});
        return null;
      }
