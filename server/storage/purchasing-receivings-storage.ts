...
const methods = {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  //  editPostedReceiving — تعديل إذن استلام مُرحَّل لم يُحوَّل بعد لفاتورة شراء
  //
  //  الخطوات:
  //   1. قفل + التحقق من الحالة (posted_qty_only فقط)
  //   2. عكس حركات المخزون القديمة (مع منع الرصيد السالب)
  //   3. حذف سطور الاستلام القديمة وإدراج الجديدة
  //   4. إعادة تطبيق المخزون بالكميات الجديدة
  //   5. إعادة ضبط journalStatus وعكس القيد القديم إن وُجد
  // ─────────────────────────────────────────────────────────────────────────
  async editPostedReceiving(
    ...
    // ── نجمع معلومات GL قبل الـ transaction لنستخدمها بعده ─────────────────
    ...
    const result = await db.transaction(async (tx) => {
      ...
      // 2. جلب حركات المخزون المرتبطة بهذا الاستلام
      ...
      // تجميع الكميات القديمة بحسب lot_id
      ...
      // عكس المخزون (مع منع الرصيد السالب — الصنف تم بيعه/صرفه)
      ...
      // 3. حذف السطور القديمة وإدراج الجديدة
      ...
      // تحديث إجماليات الرأس + إعادة ضبط journalStatus
      await tx.update(receivingHeaders).set({
        totalQty: totalQty.toFixed(4),
        totalCost: roundMoney(totalCost),
        journalStatus: 'none',
      ...
      // جلب id القيد القديم قبل نهاية الـ transaction
    ...
    // ── بعد الـ transaction: عكس القيد القديم + إنشاء قيد جديد ─────────────
    ...
    // إعادة تشغيل GL fire-and-forget (نفس منطق postReceiving)
    const recvLines = await db.select().from(receivingLines).where(eq(receivingLines.receivingId, id));
