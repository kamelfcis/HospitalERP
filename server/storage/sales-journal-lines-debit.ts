...
export interface DebitLinesResult {
  lines: InsertJournalLine[];
  nextLineNum: number;
  contractEffectiveVat: number;
}

export function buildSalesDebitLines(
  invoiceId: string,
  invoice: any,
  grossRevenue: number,
  ...
  const patientShareTotal = parseFloat(invoice.patientShareTotal || "0");
  const companyShareTotal = parseFloat(invoice.companyShareTotal || "0");
  ...
  if (canSplitAR) {
    ...
    if (totalCompanyAR > 0.001) {
      ...
      journalLineData.push({
        ...
        description: `ذمة شركة تأمين — ${invoice.contractCompany || "شركة"}`,
      });
    }
