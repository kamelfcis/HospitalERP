...
const methods = {
  ...
  async createSalesReturn(this: DatabaseStorage, data: {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      // validatedLines carries tax fields for snapshot storage on the return lines
      ...
      for (const rl of data.returnLines) {
        ...
        // ── الضريبة — snapshot نسبي من السطر الأصلي ──────────────────────
      ...
      // حقول الضريبة على مستوى الفاتورة (نسبة من الإجماليات المحسوبة لكل سطر)
