...
export interface CogsRevenueParams {
  invoiceId: string;
  invoice: any;
...
export async function buildCogsRevenueVatLines(params: CogsRevenueParams): Promise<InsertJournalLine[]> {
  const {
    invoiceId, invoice, mappingMap, inventoryAccountId,
    cogsDrugs, cogsSupplies, revenueDrugs, revenueSupplies,
    startLineNum, contractEffectiveVat,
  } = params;
  ...
  if (revenueDrugsMapping?.creditAccountId && revenueDrugs > 0.001) {
    journalLineData.push({
      journalEntryId: "", lineNumber: lineNum++,
      accountId: revenueDrugsMapping.creditAccountId,
      debit: "0", credit: String(revenueDrugs.toFixed(2)),
  ...
  if (revenueSuppliesMapping?.creditAccountId && revenueSupplies > 0.001) {
    journalLineData.push({
      ...
      debit: "0", credit: String(revenueSupplies.toFixed(2)),
      ...
      if (contractEffectiveVat >= 0) {
        if (contractEffectiveVat > 0.001) {
          journalLineData.push({
            ...
            debit: "0", credit: String(contractEffectiveVat.toFixed(2)),
        ...
        journalLineData.push({
          journalEntryId: "", lineNumber: lineNum++,
          accountId: vatOutputMapping.creditAccountId,
          debit: "0", credit: String(totalTaxAmount.toFixed(2)),
  ...
  if (hasInventoryAccount && totalCogs > 0.001) {
    journalLineData.push({
      ...
      debit: "0", credit: String(totalCogs.toFixed(2)),
