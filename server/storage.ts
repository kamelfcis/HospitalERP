...
export class DatabaseStorage implements IStorage {
  ...
  async getPatientStats(filters?: { search?: string; dateFrom?: string; dateTo?: string; deptId?: string }): Promise<any[]> {
    ...
    // ────────────────────────────────────────────────────────────────────────────
    // سياسة الـ JOIN: LEFT JOIN دائماً
    //
    //  • يظهر كل المرضى النشطين بغض النظر عن وجود فواتير في الفترة المحددة
    //  • فلتر التاريخ يؤثر فقط على المبالغ المحسوبة (داخل subquery)
    //  • المرضى الجدد بلا فواتير يظهرون بمبالغ = 0
    //  • فلتر القسم يُميّز (بـ opacity) مَن ليس لهم نشاط في هذا القسم
    // ────────────────────────────────────────────────────────────────────────────
